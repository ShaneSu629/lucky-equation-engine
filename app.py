...
model = "deepseek-ai/DeepSeek-R1"''', language="toml")
    ...
    with col_sel:
        view_name = st.selectbox("选择要查看的历史彩种", ["双色球 (ssq)", "快乐 8 (kl8)", "福彩 3D (fcsd)"])
        
    name_map = {"双色球 (ssq)": "ssq", "快乐 8 (kl8)": "kl8", "福彩 3D (fcsd)": "fcsd"}
    ...
    if ENSEMBLE_AVAILABLE:
        ...
        if st.button("🧬 运行集成预测", type="primary", use_container_width=True, key="btn_ensemble"):
            with st.spinner("🧬 正在运行四维建模 + 10,000次蒙特卡洛模拟..."):
                lt_map = {"双色球 (SSQ)": "ssq", "快乐8 (KL8)": "kl8", "福彩3D (FCSD)": "fcsd"}
